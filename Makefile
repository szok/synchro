...
.PHONY: help build build-windows test test-cover fmt fmt-check vet check clean run vscode-bin vscode-package
...
vscode-package: ## Build one VS Code extension package (.vsix) per platform into vscode/dist.
	cd vscode && npm ci
	mkdir -p vscode/dist
